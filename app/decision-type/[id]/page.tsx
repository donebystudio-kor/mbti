...
export function generateStaticParams() {
  return DECISION_TYPES.map((t) => ({ id: t.id }));
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  const { id } = await params;
  const type = DECISION_TYPES.find((t) => t.id === id);
  if (!type) return {};
  return {
    title: `${type.emoji} ${type.name} — 결정 방식 유형 | 인생재부팅연구소`,
    description: type.description,
    openGraph: {
      title: `나의 결정 방식 유형은 ${type.name}!`,
      description: type.subtitle,
    },
  };
}

export default async function DecisionTypeResultPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const type = DECISION_TYPES.find((t) => t.id === id);
  if (!type) notFound();
    ...
    <main className="min-h-screen px-4 py-12">
      <div className="max-w-sm mx-auto">
        <div className="text-center mb-8">
          <Link href="/decision-type" className="text-[#9CA3AF] text-sm hover:text-[#FF6B9D] transition-colors mb-6 inline-block">
            ← 테스트 하러 가기
          </Link>
        </div>
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5 mb-5 shadow-sm">
          <p className="text-[#9CA3AF] text-xs font-bold tracking-wider mb-3">💡 나한테 맞는 결정 팁</p>
          <p className="text-[#374151] text-sm leading-6">{type.tip}</p>
        </div>

        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5 mb-5 shadow-sm">
          <p className="text-[#9CA3AF] text-xs font-bold tracking-wider mb-3">🤔 다른 결정 유형</p>
          <div className="flex flex-wrap gap-2">
            {DECISION_TYPES.filter((t) => t.id !== id).map((t) => (
              <Link
                key={t.id}
                href={`/decision-type/${t.id}`}
