...
export function generateStaticParams() {
  return LEADER_TYPES.map((t) => ({ id: t.id }));
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  const { id } = await params;
  const type = LEADER_TYPES.find((t) => t.id === id);
  if (!type) return {};
  return {
    title: `${type.emoji} ${type.name} — 리더 타입 | 인생재부팅연구소`,
    description: type.description,
    openGraph: {
      title: `나의 리더 타입은 ${type.name}!`,
      description: type.subtitle,
    },
  };
}

export default async function LeaderTypeResultPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const type = LEADER_TYPES.find((t) => t.id === id);
  if (!type) notFound();
    ...
    <main className="min-h-screen px-4 py-12">
      <div className="max-w-sm mx-auto">
        <div className="text-center mb-8">
          <Link href="/leader-type" className="text-[#9CA3AF] text-sm hover:text-[#FF6B9D] transition-colors mb-6 inline-block">
            ← 테스트 하러 가기
          </Link>
        </div>
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5 mb-5 shadow-sm">
          <p className="text-[#9CA3AF] text-xs font-bold tracking-wider mb-3">👑 다른 리더 타입</p>
          <div className="flex flex-wrap gap-2">
            {LEADER_TYPES.filter((t) => t.id !== id).map((t) => (
              <Link
                key={t.id}
                href={`/leader-type/${t.id}`}
