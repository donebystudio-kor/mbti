    ...
    return (
      <div className="max-w-sm mx-auto">
        ...
        <div key={current} className="fade-in-up">
          ...
          {/* 채팅창 */}
          <div className="bg-white border border-[#E5E7EB] border-t-0 rounded-b-2xl px-4 pt-5 pb-5">
            {/* 내 메시지 */}
            ...
            <div className="space-y-2.5">
              {q.options.map((opt) => (
                ...
                >
                  <div className={`w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-white text-[10px] font-bold mt-0.5 transition-all ${
                    selected === opt.type
                  ...
                  <div className={`flex-1 rounded-2xl rounded-tl-sm px-4 py-2.5 transition-all duration-200 ${
                    selected === opt.type
        ...
        {current > 0 && (
          <button onClick={handleBack} className="mt-4 w-full text-[#9CA3AF] text-sm hover:text-[#6B7280] transition-colors">
            ← 이전 질문
          </button>
        )}
      </div>
