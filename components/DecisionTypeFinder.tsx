...
import { DECISION_QUESTIONS, DECISION_TYPES, type DecisionType } from "@/constants/decisionTypes";
import ResultShare from "@/components/ResultShare";

type Step = "quiz" | "result";

function calcDecisionType(answers: string[]): DecisionType {
  const counts: Record<string, number> = {};
  answers.forEach((type) => {
    counts[type] = (counts[type] || 0) + 1;
  });
  const topId = Object.entries(counts).sort((a, b) => b[1] - a[1])[0][0];
  return DECISION_TYPES.find((t) => t.id === topId)!;
}

export default function DecisionTypeFinder() {
  const [step, setStep] = useState<Step>("quiz");
  const [current, setCurrent] = useState(0);
  const [answers, setAnswers] = useState<string[]>([]);
  const [result, setResult] = useState<DecisionType | null>(null);

  const q = DECISION_QUESTIONS[current];
  const progress = ((current + 1) / DECISION_QUESTIONS.length) * 100;

  function handleAnswer(type: string) {
    const newAnswers = [...answers, type];

    if (current < DECISION_QUESTIONS.length - 1) {
      setAnswers(newAnswers);
      setCurrent(current + 1);
      return;
    }

    const decisionType = calcDecisionType(newAnswers);
    setResult(decisionType);
    setAnswers(newAnswers);
    setStep("result");
  }
      ...
      <div className="max-w-sm mx-auto">
        <div className="mb-8">
          <div className="flex justify-between text-xs text-[#9CA3AF] mb-2">
            <span>{current + 1} / {DECISION_QUESTIONS.length}</span>
