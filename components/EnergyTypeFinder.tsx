...
import { ENERGY_QUESTIONS, ENERGY_TYPES, type EnergyType } from "@/constants/energyTypes";
import ResultShare from "@/components/ResultShare";

type Step = "quiz" | "result";

function calcEnergyType(answers: string[]): EnergyType {
  const counts: Record<string, number> = {};
  answers.forEach((type) => {
    counts[type] = (counts[type] || 0) + 1;
  });
  const topId = Object.entries(counts).sort((a, b) => b[1] - a[1])[0][0];
  return ENERGY_TYPES.find((t) => t.id === topId)!;
}

export default function EnergyTypeFinder() {
  const [step, setStep] = useState<Step>("quiz");
  const [current, setCurrent] = useState(0);
  const [answers, setAnswers] = useState<string[]>([]);
  const [result, setResult] = useState<EnergyType | null>(null);

  const q = ENERGY_QUESTIONS[current];
  const progress = ((current + 1) / ENERGY_QUESTIONS.length) * 100;

  function handleAnswer(type: string) {
    const newAnswers = [...answers, type];

    if (current < ENERGY_QUESTIONS.length - 1) {
      setAnswers(newAnswers);
      setCurrent(current + 1);
      return;
    }

    const energyType = calcEnergyType(newAnswers);
    setResult(energyType);
    setAnswers(newAnswers);
    setStep("result");
  }
      ...
      <div className="max-w-sm mx-auto">
        <div className="mb-8">
          <div className="flex justify-between text-xs text-[#9CA3AF] mb-2">
            <span>{current + 1} / {ENERGY_QUESTIONS.length}</span>
  ...
  return (
    <div className="max-w-sm mx-auto fade-in-up">
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5 mb-5 shadow-sm">
        <p className="text-[#9CA3AF] text-xs font-bold tracking-wider mb-3">🌱 에너지 회복 팁</p>
      ...
      <ResultShare
        ...
        label="나의 에너지 도둑 유형"
        shareUrl={`/energy-type/${result.id}`}
      />
