...
function calcStressType(answers: string[]): StressType {
  const counts: Record<string, number> = {};
  answers.forEach((type) => {
    counts[type] = (counts[type] || 0) + 1;
  });
  const topId = Object.entries(counts).sort((a, b) => b[1] - a[1])[0][0];
  return STRESS_TYPES.find((t) => t.id === topId)!;
}

export default function StressTypeFinder() {
  const [step, setStep] = useState<Step>("quiz");
  const [current, setCurrent] = useState(0);
  const [answers, setAnswers] = useState<string[]>([]);
  const [result, setResult] = useState<StressType | null>(null);

  const q = STRESS_QUESTIONS[current];
  const progress = ((current + 1) / STRESS_QUESTIONS.length) * 100;

  function handleAnswer(type: string) {
    const newAnswers = [...answers, type];

    if (current < STRESS_QUESTIONS.length - 1) {
      setAnswers(newAnswers);
      setCurrent(current + 1);
      return;
    }

    const stressType = calcStressType(newAnswers);
    setResult(stressType);
    setAnswers(newAnswers);
    setStep("result");
  }
      ...
      <div className="max-w-sm mx-auto">
        ...
        <div className="mb-8">
          <div className="flex justify-between text-xs text-[#9CA3AF] mb-2">
            <span>{current + 1} / {STRESS_QUESTIONS.length}</span>
